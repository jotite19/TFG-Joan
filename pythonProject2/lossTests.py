...
def get_loss(rate, try_loss):
    ...
    print(try_loss)
    loss = torch.tensor([try_loss for x in range(10, 30)])
    if rate == 0:
        return loss

    size = torch.tensor([i for i in range(10, 30)]) #- 7
...
loss_temps = []
loss_list = [1 for i in range(6)]
rate_list = [0, 1, 3, 5, 7, 10]
# loss_list = [1]
# rate_list = [1]
...
s = torch.tensor([i for i in range(10, 30)])
